...
export class SeparateService implements ISeparateService {
    ...
    async getWav(videoId: string): Promise<Separate> {
        const port= this.configService.get<string>('SEPARATE_SERVER_PORT')
        const response: Separate = await (await this.axiosService.getRequest<string, Separate>('/separate', {videoId: videoId}, Number(port))).data;
        return response;
    }   
}